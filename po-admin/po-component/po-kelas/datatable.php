...
$currentRoleAccess = $tableroleaccess->findByAnd(id_level, $_SESSION['leveluser'], module, kelas);
...
if($currentRoleAccess->read_access == "Y"){
    ...
    while ( $aRow = mysql_fetch_array( $rResult ) )
    {
        ...
        for ( $i=1 ; $i<count($aColumns) ; $i++ )
        {
			...
			$strlink = preg_replace("/\/po-admin\/po-component\/po-kelas\/(datatable\.php$)/","",$str);
			if($currentRoleAccess->delete_access == "Y"){
				$tbldelete = "<a class='btn btn-xs btn-danger alertdel' id='$aRow[id_kelas]'><i class='fa fa-times'></i></a>";
			}
			$checkdata = "<div class='text-center'><input type='checkbox' id='titleCheckdel' /><input type='hidden' class='deldata' name='item[$no][deldata]' value='$aRow[id_kelas]' disabled></div>";
			$row[] = $checkdata;
			$row[] = $aRow['id_kelas'];
            $row[] = $aRow['kelas'];
